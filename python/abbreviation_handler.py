...
def regex_parser(text: str) -> Dict[str, List[str]]:
    ...
    for sent in sents:
        matches = re.findall(query_brackets, sent)
        if matches:
            for match in matches:
                ...
                if not putatives:
                    ...
                    putatives = re.findall(query_final, trim_sent, re.IGNORECASE)

                # select most probable out of putatives
                # by selecting putative closest to bracket
